  ...
  private static void saveLessonData() {
    ...
    try {
      ...
      for (Lesson lesson : lessonList) {
        ...
        // -> 수업 데이터를 한 문자열로 만들자.
        // 형식은 국제적으로 많이 사용하는
        // CSV(Comma Separator value)형식으로 만들자
    ...
    try {
      ...
      for (Member member : memberList) {
        ...
        String str = String.format("%d,%s,%s,%s,%s,%s,%s\n", member.getNo(),
            ...
            member.getPhoto(),member.getRegisteredDate(),member.getTel());
        out.write(str);
      }
  ...
  private static void saveBoardData() {
    ...
    try {
      ...
      for(Board board : boardList) {
        String str = String.format("%d,%s,%s,%d",board.getNo(),board.getContents(),
            board.getCreatedDate(),board.getViewCount());
        out.write(str);
      }
    }catch(FileNotFoundExcepthion e) {
      System.out.println("파일을 생성할 수 없다");
    }catch(IOException e) {
      System.out.println("오류");
    }finally {
      try {
