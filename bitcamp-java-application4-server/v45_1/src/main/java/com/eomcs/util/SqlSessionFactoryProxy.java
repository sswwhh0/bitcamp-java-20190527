...
public class SqlSessionFactoryProxy implements SqlSessionFactory {
  ...
  //    굳이 스레드의 보관된 SqlSession을 제거할 필요가 없다.
  // -> 스레드풀을 사용할 때는 스레드가 응답을 완료한 후 재사용되기 때문에,
  //    재사용되기 전에 다른 클라이언트의 요청을 처리하기 위해
  //    이전에 사용했던 SqlSession을 제거해야 한다.
  //    왜? 이전에 사용한 SqlsEssion은 트랜잭션을 종료하면 close()되기 때문
  ...
  public SqlSession openSession() {
    SqlSession sqlSession = localSqlSession.get();
    // 스레드에 SqlsEssion이 들어 있으면(트랜잭션 시작 상태) 그것을 그대로 리턴한다.
    if (sqlSession != null) {
      return sqlSession;
    }
